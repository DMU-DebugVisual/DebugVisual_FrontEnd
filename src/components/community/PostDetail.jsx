...
export default function PostDetail() {
    ...
    // 좋아요 수 및 내 상태 재조회 (API가 분리되어 있을 경우 가정)
    const refreshLikeStatus = async () => {
        try {
            const bust = Date.now();
            const res = await fetch(`${API_BASE}/api/posts/${id}/like/status?t=${bust}`, {
                method: "GET",
    ...
    const fetchComments = async () => {
        try {
            ...
            const bust = Date.now();
            const res = await fetch(`${API_BASE}/api/comments/${id}?t=${bust}`, {
                headers: { Accept: "application/json", Authorization: authHeader },
                cache: "no-store",
            });
